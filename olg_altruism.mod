...
Rss = (1-taucorp)*((alpha)*(A)*((Lss/Kss)^1-alpha));
...
thetaFss = (sFss+thetaEss)*(1+Rss) + omegaFss + psiFss + tautFss - cFss + wFss + ggFss;

thetaMss = (sMss+thetaFss)*(1+Rss) + omegaMss + psiMss + tautMss - cMss + wMss + ggMss + bgMss;

thetaPss = (sPss+thetaMss)*(1+Rss) + omegaPss + psiPss + tautPss - cPss + wPss + bgPss;

thetaRss = (sRss+thetaPss)*(1+Rss) + omegaRss + psiRss + tautRss - cRss + bgRss;
...
R = (1-taucorp)*((alpha)*(A)*((L/K(-1))^1-alpha));
...
REV = ((1-taucorp)*Y)-((taucorp)*(deltatau)*(K(-1)))+((tauc*C)+((tauk)*(X + S))+((taul)*W));
...
thetaF = (sF+thetaE(-1))*(1+R) + omegaF + psiF + tautF - cF + wF + ggF;

thetaM = (sM+thetaF(-1))*(1+R) + omegaM + psiM + tautM - cM + wM + ggM + bgM;

thetaP = (sP+thetaM(-1))*(1+R) + omegaP + psiP + tautP - cP + wP + bgP;

thetaR = (sR+thetaP(-1))*(1+R) + omegaR + psiR + tautR - cR + bgR;
